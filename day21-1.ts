...
function justTryEverything() {
  const maxInstructionCount = 15;
  const instructionsList = ['AND', 'OR', 'NOT'];
  const arg1List = ['A', 'B', 'C', 'D', 'T'];
  const arg2List = ['T', 'J'];

  while (true) {
    const instructionCount = Math.floor(Math.random() * maxInstructionCount);

    const inputs = Array.from({length: instructionCount}).map(() => {
