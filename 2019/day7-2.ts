...
const hyperparams = [5,6,7,8,9];
let maxSignal = -Infinity;

permutations(hyperparams).forEach(randomizedHyperparams => {
  const amplifiers = randomizedHyperparams.map(hyperparam => new IntcodeInterpreter(program, hyperparam));
  ...
  while (instruction !== 'exit') {
    amplifiers.forEach(amplifier => {
      const [nextSignal, nextInstruction] = amplifier.runTillHalt(signal);

      signal = nextSignal;
      if (nextInstruction === 'exit') {
  ...
  maxSignal = Math.max(lastESignal, maxSignal);
});
