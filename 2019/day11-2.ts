...
const paintBot = new IntcodeInterpreter(programmm, 1);
const robotPos = [0, 0];
let robotDir = 0;
...
function run() {
  ...
  tilesSet.add(JSON.stringify([0, 0]));
  colorsSet.add(JSON.stringify([0, 0]));
...
let sum = colorsSet.size;

console.log('!!!!!!', Array.from(colorsSet).map((_: string) => JSON.parse(_)));

let text = Array.from({length: 10}).map(_ => Array.from({length: 60}).map(_ => ' '));

Array.from(colorsSet).map((_: string) => JSON.parse(_)).forEach(([x, y]) => {
  text[-y][x] = '#';
});

console.log(text.map(_ => _.join('')).join('\n'));
