...
function isNetworkIdle() {
  const packetCount = computers.reduce((acc, curr) => acc + curr.incomingQueue.length, 0);
  const isNATPacketPresent = !!NAT.packet;

  return packetCount === 0 && isNATPacketPresent;
}

function kickstartNetwork() {
  if (NAT.packet.y === NAT.lastYSent) {
    console.log('answer! ', NAT.lastYSent);
    process.exit();
  }

  computers[0].incomingQueue.push(NAT.packet);
  NAT.lastYSent = NAT.packet.y;
  NAT.packet = undefined;
}
